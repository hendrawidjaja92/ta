...
<div class="judul-2 col-md-offset-2 col-sm-offset-2 col-xs-offset-2 row" data-example-id="carousel-with-captions">
    ...
    <h3 style="padding-left: 5%">Edit Kategori Barang <span class="glyphicon glyphicon-cog" aria-hidden="true"></span></h3>
    <?php foreach ($kategori_barang->result() as $k): ?>
    <?php if ($this->session->flashdata('category_success')) { ?>
        <div class="alert alert-success"> <?= $this->session->flashdata('category_success') ?> </div>
    <?php } ?>
    <?php if($pil->id_kategori_user == 1): ?>
        <?php echo form_open_multipart('admin/edit_kategori_barang/'.$k->id_kategori_barang); ?>

    <?php endif; ?>
    <?php if($pil->id_kategori_user == 3): ?>
        <?php echo form_open_multipart('seller/edit_kategori_barang/'.$k->id_kategori_barang ); ?>
    ...
    <div class="col-md-5 col-md-offset-1">
        <?php echo form_input(array(
            ...
            'class' => 'form-control',
            'value' => set_value('nama_kategori_barang', $k->nama_kategori_barang)
        )); ?>
    </div>
    <?php echo form_error('nama_kategori_barang'); ?>
    <div class="col-md-9 col-md-offset-1">
        <?php echo form_label('Status Kategori Barang :'); ?>
    </div>
    <div class="col-md-5 col-md-offset-1">
        <select id="status_kategori_barang" name="status_kategori_barang" class="form-control">
            <option value="0" <?= set_select('status_kategori_barang', '0', $k->status_kategori_barang == 0) ?>>Not Active</option>
            <option value="1" <?= set_select('status_kategori_barang', '1', $k->status_kategori_barang == 1) ?>>Active</option>
            <option value="2" <?= set_select('status_kategori_barang', '2', $k->status_kategori_barang == 2) ?>>Banned</option>
            <option value="3" <?= set_select('status_kategori_barang', '3', $k->status_kategori_barang == 3) ?>>Pending</option>
        </select>
